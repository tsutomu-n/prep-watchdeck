...
- `5m / 15m / 1h / 4h / 1D`のChart。初回最大500本、過去へスクロールして履歴を追加
...
取得します。Collectorの保存期間とは独立して、取引所が配信する時間足ごとの履歴を表示します。
...
| Universeの行を選ぶ | WebがそのVenueのChart履歴を取得し、500ms後にlocalhostの選択commandを書いて詳細監視を切り替える | Chart、板、約定、板上概算が更新される |
| Chartの時間足を変える | その時間足の履歴を取得する | 選んだ銘柄の最新約120本を表示する |
| Chartを過去へスクロールする | 古い履歴を追加取得する | 見ていた足と倍率を保って過去へ進める |
| 「騰落率の基準」を変更する | 時刻をBrowserに保存し、指定したJST時刻の基準価格を取得する | 一覧と選択銘柄の約定騰落率を新しい基準で表示する |
...
- Chart履歴は表示中のtabで60秒ごとに更新します。取得失敗はChart内に表示し、古い価格を最新扱いしません。
...
「騰落率の基準」は日本時間で00:00〜23:59を1分単位に設定できます。初期値は00:00です。
選択は同じBrowserに保存され、再読込後も保持します。保存できない場合はその旨を表示し、
現在の画面内では変更した時刻を使います。

約定騰落率は、指定時刻の直前の確定1分足終値を基準に、同じVenue・同じ契約の最新約定価格から
`(最新約定価格 / 基準価格 - 1) × 100`で計算します。たとえば基準が00:00なら日本時間の当日0時、
基準が09:00で現在が08:00なら前日9時が比較の起点です。基準時刻を迎えると日次の基準を切り替えます。
選択銘柄の詳細では、実際の基準日時・基準価格・約定価格・取得時刻も確認できます。

一覧で見えている行と選択銘柄だけを約1分ごとに更新します。Markや参考中央値を計算に混ぜません。
基準の1分足がなければ「基準足なし」、最新約定足がなければ「約定足なし」、古ければ「約定足が古い」と
表示します。欠測を0%や別の時間の価格で埋めません。MobileではMarkの下に約定騰落率を表示します。
この設定は騰落率の比較基準であり、Chartの1Dの区切りを変更するものではありません。
...
Chartは選択した`venueInstrumentId`だけを描画します。時間足を変えても選択中のInstrumentは変わらず、
銘柄やVenueを変えても選択した時間足を保持します。

`5m`は5分足、`1h`は1時間足、`1D`は1日足で、1本が表す長さです。画面下の「表示期間」は
実際に見えている範囲です。ズームやドラッグで範囲を変え、「最新へ」で最新位置へ、
「全体表示」で読み込み済み全体へ移動できます。定期更新でズームを解除しません。

初回は最大500本を取得し、最新約120本を表示します。過去へのスクロール、または
「さらに過去を読み込む」で追加できます。1銘柄・1時間足につき最大10,000本で、実際に取得できる
長さは取引所の履歴配信範囲によります。過去8日固定ではありません。
時刻はチャート・周辺情報ともJSTです。日足はUTC日付で区切るため、JSTでは09:00に始まります。
...
| 約定騰落率 | 指定したJST時刻直前の確定1分足終値からの変動率 | 同じ契約の約定価格同士で計算。Markの騰落率や24時間前との比較ではない |
...
- Chartの配信されない足は補間しません。未確定の足は取引所からの更新で変わります。
- 画面のChartは取引所が配信するsymbolの履歴です。Collectorの保存履歴・version判定とは別です。
...
Universe Explorerの「デイトレランキング」から開きます。Bitget・Hyperliquid・Asterの取扱い銘柄を、
確認済みの外部参照契約で比較する画面です。専用の収集processが必要です。起動方法は
[/home/tn/projects/prep-watchdeck/.ai-work/ranking-chart-release-20260916-2117/docs/current/operations.md](operations.md)
の「独立ランキングの起動・停止・復旧」を参照してください。

| 入力 | 起きること | 確認して次へ進む条件 |
| --- | --- | --- |
| 比較期間を15分・1時間・JST基準から選ぶ | 同じ終了時刻までの騰落率と売買代金へ切り替わる | 比較時刻と比較可能数が表示される |
| 上昇率・下落率・売買代金順を選ぶ | 全対応銘柄から並び替える | 現在の条件での順位件数を確認する |
| 売買代金の下限をUSDTで入力する | 指定額以上の参照契約へ絞り、順位を付け直す | 絞込み後の順位であることを確認する |
| JST HH:mmを変更する | 最後に到来した基準から比較する。設定はBrowserへ保存する | 開始直後や履歴不足の表示では数値を待つ |
| 銘柄名を選ぶ | 同じ参照契約を1つのTradingViewチャートへ表示する | 参照取引所・契約名を確認する |
| チャートの足を変更する | 足間隔だけを変える | 比較期間は変更されない |
| 順位外・未対応も表示する | 履歴不足・要確認・対象外も一覧へ含める | 各行の理由を読み分ける |

売買代金は、表示した外部取引所の当該契約のUSDT建て合計です。元の3取引所や市場全体の合計ではありません。
行の「取扱い」は元の3取引所、「参照」は計算・Chartの取引所です。数量倍率が異なる契約では、
「元の取扱い契約と数量単位」も確認できます。

ランキングは確定1分足で毎分更新し、全行を同じ比較時刻へそろえます。Widgetには進行中の足も含まれるため、
その瞬間のChart値とランキングの比較時刻は異なります。並び替えや更新で選択は移動しません。
選択した銘柄が条件外になった場合も表示を維持し、対応表から削除された場合はChartを停止します。
削除後に期間・並び順・下限を変えても、読み込み待ちや取得失敗中に旧Chartへ戻りません。
Widgetを表示できないときは「チャートを再読込み」を使えます。
次回も使う足間隔はアプリの「チャートの足」で設定してください。Widget内部で変更した日付範囲や
足間隔はアプリ設定には保存されず、再読込み時はアプリの設定に戻ります。

未対応銘柄では、Bybit・Binanceに同一資産の対応契約がない、参照契約が取扱い終了・清算中、
同名の参照先が株式の別資産である、などの理由を一覧と選択後の説明で確認できます。
同一資産・参照契約の確認が済んでいない銘柄は、確定した未対応と分けて要確認と表示します。
元契約の数量換算だけが未確認でも、確認済み参照契約のデータがそろえば順位と数値を表示します。
選択詳細には換算未確認を表示するため、元契約の数量・価格への換算に使わないでください。
Chart対応も別の確認項目です。未確認ならChartを止めますが、ランキングの数値は利用できます。
「対応範囲」で数量換算とChartの未確認件数を確認できます。

「最新の確定足が未着」「期間内の履歴不足」「参照データの取得停止」は0%ではありません。
「更新が停止しています」では最後の比較時刻を確認してください。名簿の更新停止も別に表示されます。
上昇・下落に該当する銘柄がない場合は空の順位になります。0%は売買代金順で確認できます。


### 順位の動きと値動きの背景を読む

行の順位の下に、同じ条件で比較した1分前からの順位変化を表示します。8位から3位なら`+5`、
3位から8位なら`−5`、同順位は`0`です。「新規」は、比較元の世代が存在し、前回だけ順位外だった
銘柄です。初回・再起動後、前の分の世代欠落、対応表や指標定義の変更、古い結果では理由付きの
「比較不可」になります。順位外の行では現在の除外理由を確認してください。
期間・並び順・売買代金下限・JST設定を変えると、前回と今回の両方へ同じ条件を適用します。

売買代金の下の「平常比」は、直近24時間内の同期間中央値との比較です。最新15分の売買代金が200、
それを除いた過去95個の15分窓の中央値が100なら2.0倍です。1時間では過去23個の1時間窓を使います。
最新が0で中央値が正なら0倍、中央値が0なら「比較基準なし」、全24時間の足がそろわなければ
「履歴不足」です。JST基準の可変期間には適用しません。

騰落率の下の「当日位置」はJST 00:00から比較時刻までの確定1分足のhigh/lowを使います。
安値80・高値120・終値100なら50%、安値なら0%、高値なら100%です。任意HH:mmの設定とは独立し、
日付変更直後は「開始直後」、高値と安値が同じなら「値幅なし」、足の欠落は「履歴不足」です。

銘柄を選ぶと同じ指標を詳細でも確認できます。追加指標だけが履歴不足でも、既存の騰落率・
売買代金が有効なら順位は維持します。平常比と当日位置は並び順や売買推奨を追加するものではありません。
